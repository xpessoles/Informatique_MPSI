...
Dans le cadre du modèle de la machine de von Neumann, un\textbf{ seul programme s’exécute}. Il s'agit donc du système d'exploitation qui est chargé dès le démarrage.  Ce programme a pour but de gérer les accès aux processeurs et à la mémoire et aux périphériques, ce qui permet
...
\begin{itemize}
\item sous UNIX, il existe des distributions dont les sources sont libres (modifiables par tous);
\item sous UNIX, l'accès à un shell, permet de réaliser une multitude de tâches en ligne de commande, ce qui est parfois plus efficace qu'avec la souris;
...
\begin{itemize}
	...
	\item on peut ainsi remonter dans l'arborescence jusqu'à un répertoire appelée {répertoire racine}.
\end{itemize}
...
\item \texttt{$a$/$b$}: fichier ou sous-répertoire $b$ d'un sous-répertoire $a$ (d'emplacement non précisé), c'est donc un chemin relatif. Par exemple, si le répertoire courant est \texttt{/home/adent/} et qu'une commande fait référence à \texttt{Documents/trombi.pdf}, le fichier concerné se trouve au chemin absolu \texttt{/home/adent/Documents/trombi.pdf} !
\end{itemize}
...
\end{itemize}
Ces métadonnées sont utilisées par le système pour accorder/refuser l'accès, pour savoir ce
...
$$
-\underbrace{\text{rw}-}_{1}\underbrace{\text{r}--}_{2}\underbrace{\text{r}--}_{3}
$$
\begin{itemize}
\item 1 : droits pour l'utilisateur pessoles;
\item 2 : droits pour le groupe d'utilisateurs user;
...
\begin{itemize}
\item \textbf{les programmes en langage machine} qui contiennent le code des
  instructions du processeur qu'il faut mettre en mémoire pour
  exécuter le programme;
\item \textbf{les scripts} qui sont des programmes sous forme de texte dans un langage
  humainement compréhensible. Techniquement, le système va lancer un
  programme appelé {interpréteur} et lui donner ce
...
\item En général produits par un {compilateur}, programme
...
\item Pour réaliser un script:
  ...
  \item ajouter en première ligne :
    \verb+#!+ {chemin-vers-l'interpréteur}
...
\bibitem{martins}{Ressources des enseignants du lycée La Martinière Monplaisir.}
\end{thebibliography}
