\question\ Écrire une fonction \pyv{comb(n,p)} calculant $\displaystyle\binom{n}{p}$ pour deux entiers naturels $n,p$ vérifiant $0 \leq p \leq n$, en utilisant la formule :
    \begin{equation*}
      \binom{n}{p} = \dfrac{n}{p} \times \dfrac{n-1}{p-1} \times \dots \times \dfrac{n-p+1}{1}.
    \end{equation*}
    On veillera à ce que le résultat donné par la fonction \pyv{comb} soit d'un type convenable. 
