...
## Question 10
Q10_req = "select avg(LIF_remise_pourcent) as moyenne_pourcent, avg(LIF_remise_montant) as remise_montant from T_LIGNE_FACTURE;"
Q10_res = "32.0 ; 66.0"
...
## Question 15
Q15_req = "select T_CLIENT.CLI_NOM,T_CLIENT.CLI_PRENOM, max(nb_reduc) from (select T_FACTURE.CLI_ID,count(T_FACTURE.CLI_ID) as nb_reduc from T_FACTURE join T_LIGNE_FACTURE on T_LIGNE_FACTURE.FAC_ID=T_FACTURE.FAC_ID where LIF_REMISE_MONTANT>0 or LIF_REMISE_POURCENT>0 group by CLI_ID) as c join T_CLIENT on T_CLIENT.CLI_ID=c.CLI_ID;"
