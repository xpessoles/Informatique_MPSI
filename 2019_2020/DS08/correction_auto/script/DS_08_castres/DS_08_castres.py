...
## Question 6
Q6_req = "SELECT CLI_NOM as Noms, CLI_PRENOM as Prénoms from (SELECT CLI_NOM, CLI_PRENOM, TIT_CODE from T_CLIENT where TIT_CODE='Mme. or TIT_CODE='Melle.') order by CLI_NOM ASC;"
...
## Question 13
Q13_req = "SELECT DISTINCT CLI_ID from T_FACTURE JOIN T_LIGNE_FACTURE on T_LIGNE_FACTURE.FAC_ID=T_FACTURE.FAC_ID where LIF_REMISE_POURCENT>0 or LIF_REMISE_MONTANT>0;"

## Question 14
Q14_req = "SELECT CLI_ID from T_FACTURE except SELECT DISTINCT CLI_ID from T_FACTURE JOIN T_LIGNE_FACTURE on T_LIGNE_FACTURE.FAC_ID=T_FACTURE.FAC_ID where LIF_REMISE_POURCENT>0 or LIF_REMISE_MONTANT>0;"


## Question 15
Q15_req = "select T_CLIENT.CLI_NOM, T_CLIENT.CLI_PRENOM, max(nbred) from (select T_FACTURE.CLI_ID, count(T_FACTURE.CLI_ID) as nbred from T_FACTURE join T_LIGNE_FACTURE on T_LIGNE_FACTURE.FAC_ID=T_FACTURE.FAC_ID where LIF_REMISE_POURCENT>0 or LIF_REMISE_MONTANT>0 group by CLI_ID) as tout join T_CLIENT on T_client.CLI_ID=tout.CLI_ID;"
