...
## Question 3
sol_Q3_req = "SELECT CLI_NOM, CLI_PRENOM FROM T_CLIENT WHERE TIT_CODE='Mme.'"
...
## Question 10
sol_Q10_req = " SELECT AVG(LIF_REMISE_POURCENT), AVG(LIF_REMISE_MONTANT) FROM T_LIGNE_FACTURE"
#sol_Q10_res = "Montant : 50, pourcent : 15"
sol_Q10_res = str(renvoie_res_requete(sol_Q10_req))

## Question 11
sol_Q11_req = " SELECT AVG(LIF_REMISE_POURCENT), MAX(LIF_REMISE_MONTANT) FROM T_LIGNE_FACTURE"
#sol_Q11_res = "Montant : 50, pourcent : 15"
sol_Q11_res = str(renvoie_res_requete(sol_Q11_req))

## Question 12
sol_Q12_req = "SELECT DISTINCT FAC_ID FROM T_LIGNE_FACTURE WHERE LIF_REMISE_POURCENT>0 OR LIF_REMISE_MONTANT>0"
...
## Question 15
sol_Q15_req = "SELECT DISTINCT T_CLIENT.CLI_NOM, T_CLIENT.CLI_PRENOM, max(montant_total) FROM T_CLIENT JOIN (SELECT CLI_ID as id_client, sum(montant) as montant_total FROM T_FACTURE JOIN (SELECT T_LIGNE_FACTURE.FAC_ID as facid1,  T_LIGNE_FACTURE.LIF_REMISE_MONTANT as montant FROM T_LIGNE_FACTURE WHERE LIF_REMISE_POURCENT>0 OR LIF_REMISE_MONTANT>0)ON facid1=T_FACTURE.FAC_ID GROUP BY CLI_ID) ON T_CLIENT.CLI_ID=id_client;"



sol_Q15_req2 = "select T_CLIENT.CLI_NOM,T_CLIENT.CLI_PRENOM, max(nb_reduc) from (select T_FACTURE.CLI_ID,count(T_FACTURE.CLI_ID) as nb_reduc from T_FACTURE join T_LIGNE_FACTURE on T_LIGNE_FACTURE.FAC_ID=T_FACTURE.FAC_ID where LIF_REMISE_MONTANT>0 or LIF_REMISE_POURCENT>0 group by CLI_ID) as c join T_CLIENT on T_CLIENT.CLI_ID=c.CLI_ID;"
#sol_Q15_res ="Medard,Jacques,600"
sol_Q15_res = str(renvoie_res_requete(sol_Q15_req))

sol_Q15_res2 = str(renvoie_res_requete(sol_Q15_req2))
...
def test_Q15_res2 ():
    assert sol_Q15_res2.upper() ==  Q15_res.upper()

def test_Q15_req2 ():
    assert requete(sol_Q15_req2) == requete(Q15_req)